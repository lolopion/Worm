...
enum ResCodes doLevel(struct game_options* somegops, char* level_filename) {
    ...
    res_codes = initializeLevelFromFile(&theboard, level_filename);
    if (res_codes != RES_OK) {
      return res_codes;
    }
...
if (thegops.start_level_filename != NULL) {
 res_codes = doLevel(&thegops,thegops.start_level_filename);
 free(argv);
 } else {
  res_codes = doLevel(&thegops, "basic.level.1");
 }
return res_codes;
}
